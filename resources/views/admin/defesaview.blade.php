                ...
                <div class="container-fluid">
                    <div class="card shadow">
                        <div class="card-header py-3 d-md-flex d-lg-flex d-xl-flex">
                            <h2 class="d-xl-flex mr-auto" style="color: #3ba2dc;font-family: Roboto, sans-serif;">Defesas</h2>
                            <p class="" style="color:#2694c1;font-size:14px"><strong>Já forma marcadas mais de {{count($defesas)}} defesas</strong></p>
                        </div>

                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 col-xl-5 offset-xl-0 justify-content-end align-items-center" style="width: 100%;">
                                    <form method="get" action="{{ ('/defesa/index') }}" enctype="multipart/form-data">
                                        <div class="text-md-right justify-content-start align-items-center dataTables_filter" id="dataTable_filter" style="width: 100%;padding-right: 10px;padding-left: 10px;height: 50px;">
                                            <label style="width: 100%;">
                                                <input class="border rounded border-primary shadow form-control form-control-sm" type="search" aria-controls="dataTable" id="search" placeholder="Pesquisar defesa pelo nome do estudante..." style="padding-bottom: 0px;height: 40px;" autocomplete="on" name="search">
                                            </label>
                                        </div>
                                    </form>
                                </div>
                                <div class="col-md-6 col-xl-7 offset-xl-0 d-flex justify-content-end align-items-center">
                                    ...
                                    <div class="btn-group" role="group">
                                        <a class="btn border rounded d-xl-flex align-items-xl-center" role="button" style="background-color: #3ba2dc;height: 40px;color: #ffffff;" href="{{ route('newdefesa') }}">
                                            <i class="fas fa-book"></i>&nbsp; Nova Defesa<br></a>
                                    </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <main>
                                ...
                                <div class="container-fluid" role="alert" style="width: 100%;padding-top: 8px;padding-bottom: 8px;">
                                    <div class="mensagem">
                                        ...
                                        <p class="msg"><Strong>{{ session('msgSucessDelete') }}</Strong></p>
                                        @endif
                                    </div>
                                </div>
                            </main>
                            ...
                            <div class="table-responsive" data-aos="fade-up" data-aos-duration="800" data-aos-delay="800">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="color: #2694c1;"><strong>Sala</strong></th>
                                            <th style="color: #2694c1;"><strong>Participantes</strong></th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="d-flex align-items-center" style="color: #3a3a3b;min-width:200px"><img class="rounded-circle img-perfil" src="/ficheiros/estudantes/fotos/{{$defesa->foto}}" width="40px" style="margin-right: 10px;" height="40px">{{$defesa->autor}}</td>
                                            <td style="color: #3a3a3b;">{{$defesa->tema}}</td>
                                            <td style="color: #3a3a3b;">{{$defesa->curso}}</td>
                                            <td style="color: #3a3a3b;">{{$defesa->nivel}}<br></td>
                                            <td style="color: #3a3a3b;">{{$defesa->supervisor}}</td>
                                            <td style="color: #3a3a3b;">{{$defesa->sala}}</td>
                                            <td style="color: #3a3a3b;text-align:center;">{{count($defesa->participanteDefesa)}}</td>
                                            <td>
                                                <div class="col d-flex  align-items-start align-content-start">
                                                    @if(auth()->user()->is_drcurso)
                                                    <a class="btn btn-sm d-flex align-items-center btn-actualizar" href="/defesa/edit/{{ $defesa->id }}" type="button"><ion-icon name="create"></ion-icon>&nbsp;Editar</a>
                                                    <form action="/defesa/delete/{{ $defesa->id }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                    ...
                                                    <!--<a class="btn btn-sm d-flex align-items-center btn-imprimir" type="button" href="/defesa/showimprim/{{$defesa->id}}"><ion-icon name="print-outline"></ion-icon>&nbsp;Imprimir</a>-->
                                                        <a class="btn btn-sm d-flex align-items-center btn-imprimir" type="button" href="/defesa/imprimi/{{$defesa->id}}"><ion-icon name="print-outline"></ion-icon>&nbsp;Imprimir</a>
                                                    
                                                    @endif
                                                    <a class="btn btn-sm d-flex align-items-center btn-ver" type="button" href="/defesa/showadmin/{{ $defesa->id }}"><ion-icon name="eye"></ion-icon>&nbsp;Ver</a>
                                                </div>
                                            </td>
                                        </tr>
                                        @endforeach
                                ...
                                <div class="container-fluid">
                                    ...
                                    <p class="msg" style="color:#2390be;"><Strong>Não existe nenhuma defesa com o nome {{$search}}!</Strong><a style="font-size:16px" href="{{ ('/defesa/index') }}"> Ver todas defesas</a></p>
                                    @elseif(count($defesas) == 0)
                                    <p class="msg"><Strong>Nenhuma defesa cadastrada</Strong></p>
                                    @endif
                                </div>
                            </div>
                            <!--<div class="row">
