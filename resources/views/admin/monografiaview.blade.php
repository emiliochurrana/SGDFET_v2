                        ...
                        <!--<li class="nav-item dropdown no-arrow mx-1" role="presentation">
                            <div class="nav-item dropdown no-arrow">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-list dropdown-menu-right animated--grow-in" role="menu">
                                    ...
                                    <a class="text-center dropdown-item small text-gray-500" href="{{('/notificacoes')}}">Todas notificacoes</a>
                                </div>
                            </div>
                        </li>
                        <li class="nav-item dropdown no-arrow mx-1" role="presentation">
                            <div class="nav-item dropdown no-arrow">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-list dropdown-menu-right animated--grow-in" role="menu">
                                    ...
                                    <a class="text-center dropdown-item small text-gray-500" href="">Todas notificacoes</a>
                                </div>
                            </div>
            ...
            <div class="container-fluid">
                <div class="card shadow">
                    <div class="card-header py-3 d-md-flex d-lg-flex d-xl-flex">
                        <h2 class="d-xl-flex mr-auto" style="color: #3ba2dc;font-family: Roboto, sans-serif;">Monografias</h2>
                        <p class="" style="color:#2694c1;font-size:14px"><strong>Existe um total de {{count($monografias)}} monografias publicadas</strong></p>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-xl-5 offset-xl-0 justify-content-end align-items-center" style="width: 100%;">
                                <form method="get" action="{{ ('/monografias/index') }}" enctype="multipart/form-data">
                                    <div class="text-md-right justify-content-start align-items-center dataTables_filter" id="dataTable_filter" style="width: 100%;padding-right: 10px;padding-left: 10px;height: 50px;">
                                        <label style="width: 100%;">
                                            <input class="border rounded border-primary shadow form-control form-control-sm" type="search" aria-controls="dataTable" id="search" placeholder="Pesquisar monografia pelo nome do autor..." style="padding-bottom: 0px;height: 40px;" autocomplete="on" name="search">
                                        </label>
                                    </div>
                                </form>
                            </div>
                            <div class="col-md-6 col-xl-7 offset-xl-0 d-flex justify-content-end align-items-center">
                                ...
                                <div class="btn-group" role="group">
                                    <a class="btn border rounded d-xl-flex align-items-xl-center" role="button" style="background-color: #3ba2dc;height: 40px;color: #ffffff;" href="{{ route('newmonografia') }}">
                                        <i class="fas fa-book"></i>&nbsp; Nova Monografia<br></a>
                                </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="table-responsive" data-aos="fade-up" data-aos-duration="800" data-aos-delay="800">
                            <table class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="color: #2694c1;"><strong>Tema</strong></th>
                                        <th style="color: #2694c1;"><strong>Curso</strong></th>
                                        <th style="color: #2694c1;"><strong>Nivel</strong></th>
                                        <th style="color: #2694c1;"><strong>Supervisor</strong></th>
                                        <th style="color: #2694c1;"><strong>Data</strong></th>
                                        <th style="color: #2694c1;"><strong>Hora</strong></th>
                                        <th style="color: #2694c1;"><strong>Downloads</strong></th>
                                        <th class="text-left" style="color: #2694c1;"><strong>Acção</strong><br></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($monografias as $mono)
                                    <tr>
                                        <td class="d-flex align-items-center" style="color: #3a3a3b;min-width:200px;"><img class="rounded-circle img-perfil" src="/ficheiros/estudantes/fotos/{{$mono->foto}}" style="margin-right: 10px;">{{$mono->autor}}</td>
                                        <td style="color: #3a3a3b;">{{$mono->tema}}</td>
                                        <td style="color: #3a3a3b;">{{$mono->curso}}</td>
                                        <td style="color: #3a3a3b;">{{$mono->nivel}}<br></td>
                                        <td style="color: #3a3a3b;">{{$mono->supervisor}}</td>
                                        <td style="color: #3a3a3b;"> {{date('d/m/y', strtotime($mono->created_at))}}</td>
                                        <td style="color: #3a3a3b;"> {{date('h:i', strtotime($mono->created_at))}}</td>
                                        <td style="color: #3a3a3b;text-align:center"> {{count($mono->downloadMonografia)}}</td>

                                        <td>
                                            <div class="col d-flex align-items-center align-content-center">
                                                @if(auth()->user()->is_drcurso)
                                                <a class="btn btn-sm d-flex align-items-center" type="button" href="/monografia/edit/{{$mono->id}}" style="background-color: #0ccf94;color: rgb(242,244,245);margin-right: 10px;margin-top: 6px;"><ion-icon name="create"></ion-icon>&nbsp;Editar</a>
                                                <form action="/monografia/delete/{{$mono->id}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                ...
                                                @endif
                                                <a class="btn btn-sm d-flex align-items-center" type="button" href="/monografia/showadmin/{{$mono->id}}" style="background-color: #0280c6;color: rgb(243,246,248);margin-top: 6px;"><ion-icon name="eye"></ion-icon>&nbsp;Ver</a>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
                            ...
                            <div class="container-fluid">
                                ...
                                <p class="msg" style="color:#2390be;"><Strong>Não existe nenhuma monografia com o autor {{$search}}!</Strong><a style="font-size:16px" href="{{ ('/monografias/index') }}"> Ver todas monografias</a></p>
                                @elseif(count($monografias) == 0)
                                <p class="msg"><Strong>Nenhuma monografia cadastrada</Strong></p>
                                @endif
                            </div>
                        </div>
                        <!--<div class="row">
